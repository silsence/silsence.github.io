...
<h4 id="引用捕获">引用捕获 </h4>
<pre class="language-text">与引用传参类似，引用捕获保存的是引用，值会发生变化。
...
<pre class="language-text">在C++中,可以使用new来申请堆区内存空间,采用delete释放堆区内存空间
...
<h4 id="malloc和new的区别">malloc和new的区别 </h4>
<pre class="language-text">malloc需要手动计算申请空间的大小，new只需要在数据类型后面的[]内输入所要申请类型空间的个数即可
...
<h4 id="智能指针">智能指针 </h4>
<h3 id="类">类 </h3>
<h4 id="构造的多种方法">构造的多种方法 </h4>
<pre class="language-text">- name p1;//默认构造
- name p2(20);//有参构造
- name p3(p2);//拷贝构造
